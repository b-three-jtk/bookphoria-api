                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <span aria-current="page" class="flex items-center justify-center px-3 h-8 text-polo-blue-600 border border-gray-300 bg-polo-blue-50 hover:bg-polo-blue-100 hover:text-polo-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{{ $page }}</span>
                        @else
